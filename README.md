...
5. Create one technician user and one analyst user in `User Management`.
6. Sign out and sign in as the technician.
7. Open `Upload & Analyze`.
8. Upload [thesis-smoke.wav](C:/Users/Prog/Desktop/FYP-2/sample-data/thesis-smoke.wav) or your own WAV/MP3 call recording.
9. Wait for the case detail page.
10. Verify:
   - transcript is visible
   - waveform area loads
   - audio score, text score, confidence, and risk level are visible
   - PDF export works
11. Sign out and sign in as the analyst.
12. Open `Review & Validation`, open the case, and save a review decision.
13. Sign back in as administrator.
14. Confirm the result in `Dashboard`, `Case History`, `Reports & Analytics`, and `Audit Logs`.
